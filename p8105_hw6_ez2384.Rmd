...
```{r Bootstrap Samples, message=FALSE, warning=FALSE}
...
```{r Distribution Plots, message=FALSE, warning=FALSE, fig.width=8, fig.height=8}
...
```{r GLM for each city, warning=FALSE, message=FALSE}
...
##### Building and Commentting on Reduced Model          
...
```{r birthweight models for comparison}
...
```{r, cross-validation, message=FALSE, warning=FALSE}
  ...
  rmse = function(model, data) { sqrt(mean((data$bwt - predict(model, newdata = data))^2)) }
...
```{r prob3 plot, fig.height=7, fig.width=7}
